...
$guides = function () {
    ...
    foreach ($ranks as $rankName => $rankDatas) {
        $guides[] = array(
            ...
            'lineAlpha' => 1,
            'lineThickness' => 1,
            'value' => $rankDatas['from'],
            'toValue' => $rankDatas['to'],
            'valueAxis' => 'pointsAxis',
            'label' => $rankName,
            'inside' => true,
            'position' => 'right',
            'fillColor' => $guideColors[$i++ % count($guideColors)]
        );
    }
...
$datas = function () {
    ...
    foreach ($files as $file) {
        if(!isset($_GET['all']) && time() - (explode('.', array_values(array_slice(explode('/', $file), -1))[0])[0] / 1000) > 604800)
            continue;
